...
{
    SimMutex_lock(me->itsSimMutex);
    GPSPositionSensor_activaete(me->itsGPSPositionSensor);
    SimMutex_release(me->itsSimMutex);
}
...
void SensorMaster_optical_getSpeed(SensorMaster* const me)
{
    double speed;
    SimMutex_lock(me->itsSimMutex);
...
void SensorMaster_setItsSimMutex(const SensorMaster* const me)
{
    return (struct SimMutex*)me->itsSimMutex;
}
